...
//                SpannableString spannableString = ((SpannableString) mDataList.get(i));
//
////                int startIndex = 0;
////                int endIndex = ((SpannableString) mDataList.get(i)).length();
////                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
////                    textPaint.getTextBounds(spannableString, startIndex, endIndex, textBounds);
////                }
...
//            }
    ...
    private void drawSelectedRect(Canvas canvas) {
        if (isDrawSelectedRect) {
            textPaint.setColor(mSelectedRectColor);
            canvas.drawRect(mClipLeft, mSelectedItemTopLimit, mClipRight, mSelectedItemBottomLimit, textPaint);
        }
    }
    ...
    private void drawDivider(Canvas canvas) {
        if (isShowDivider) {
            textPaint.setColor(mDividerColor);
            float originStrokeWidth = textPaint.getStrokeWidth();
            textPaint.setStrokeJoin(Paint.Join.ROUND);
            textPaint.setStrokeCap(Paint.Cap.ROUND);
            textPaint.setStrokeWidth(mDividerSize);
            if (mDividerType == DIVIDER_TYPE_FILL) {
                canvas.drawLine(mClipLeft, mSelectedItemTopLimit, mClipRight, mSelectedItemTopLimit, textPaint);
                canvas.drawLine(mClipLeft, mSelectedItemBottomLimit, mClipRight, mSelectedItemBottomLimit, textPaint);
            } else {
                ...
                canvas.drawLine(wrapStartX, mSelectedItemTopLimit, wrapStopX, mSelectedItemTopLimit, textPaint);
                canvas.drawLine(wrapStartX, mSelectedItemBottomLimit, wrapStopX, mSelectedItemBottomLimit, textPaint);
            }
            textPaint.setStrokeWidth(originStrokeWidth);
        }
    }
    ...
    private void drawItem(Canvas canvas, int index, int scrolledOffset) {
        ...
        if (Math.abs(item2CenterOffsetY) <= 0) {
            ...
            clipAndDraw2DText(canvas, spannableString, mSelectedItemBottomLimit, mClipBottom, item2CenterOffsetY, centerToBaselineY);
            ...
            //mIsBoldForSelectedItem==true 改变字体
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw2DText(canvas, spannableString, mClipTop, mSelectedItemTopLimit, item2CenterOffsetY, centerToBaselineY);
            textPaint.setTextSize(textSize);
            //mIsBoldForSelectedItem==true 恢复字体
            resetTypefaceIfBoldForSelectedItem();
        } else {
            ...
            //mIsBoldForSelectedItem==true 改变字体
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw2DText(canvas, spannableString, mClipTop, mClipBottom, item2CenterOffsetY, centerToBaselineY);
            textPaint.setTextSize(textSize);
            //mIsBoldForSelectedItem==true 恢复字体
            resetTypefaceIfBoldForSelectedItem();
        }
    ...
    private void clipAndDraw2DText(Canvas canvas, SpannableString text, int clipTop, int clipBottom,
        ...
        canvas.translate(0, mCenterY + item2CenterOffsetY - centerToBaselineY);

        TextPaint tp = new TextPaint(Paint.ANTI_ALIAS_FLAG);

        //text.setSpan(new LinearGradientSpan(), 0, text.length(), SPAN_INCLUSIVE_INCLUSIVE);
        StaticLayout layout = createStaticLayout(text, getWidth(), tp);
                //new StaticLayout(text, tp, (int) StaticLayout.getDesiredWidth(text, textPaint), Layout.Alignment.ALIGN_NORMAL, 1, 0, false);
        layout.draw(canvas);

        canvas.restore();
    }

    class LinearGradientSpan extends CharacterStyle implements UpdateAppearance {
        @Override
        public void updateDrawState(TextPaint textPaint) {
            ...
            textPaint.setShader(
                    new LinearGradient(
                            ...
                            0f,
                            0f,
                            ContextCompat.getColor(getContext(), R.color.test_color),
                            ContextCompat.getColor(getContext(), R.color.test_color_with_alpha),
    ...
    private int remeasureTextSize(CharSequence contentText) {
        float textWidth = textPaint.measureText(contentText, 0, contentText.length());
    ...
    private void draw3DItem(Canvas canvas, int index, int scrolledOffset) {
        ...
        if (Math.abs(item2CenterOffsetY) <= 0) {
            ...
            clipAndDraw3DText(canvas, spannableString, mSelectedItemTopLimit, mSelectedItemBottomLimit,
            ...
            clipAndDraw3DText(canvas, spannableString, mSelectedItemBottomLimit, mClipBottom,
            ...
            //字体变化，重新计算距离基线偏移
            int reCenterToBaselineY = recalculateCenterToBaselineY();
            clipAndDraw3DText(canvas, spannableString, mClipTop, mSelectedItemTopLimit,
                    rotateX, translateY, translateZ, reCenterToBaselineY);
            textPaint.setTextSize(textSize);
            //mIsBoldForSelectedItem==true 恢复字体
            resetTypefaceIfBoldForSelectedItem();
            ...
            //字体变化，重新计算距离基线偏移
            int reCenterToBaselineY = recalculateCenterToBaselineY();
            clipAndDraw3DText(canvas, spannableString, mClipTop, mClipBottom,
                    rotateX, translateY, translateZ, reCenterToBaselineY);
            textPaint.setTextSize(textSize);
            //mIsBoldForSelectedItem==true 恢复字体
            resetTypefaceIfBoldForSelectedItem();
     ...
     * @param centerToBaselineY 文字中心距离baseline的距离
     */
    private void clipAndDraw3DText(Canvas canvas, SpannableString text, int clipTop, int clipBottom,
     ...
     * @param centerToBaselineY 文字中心距离baseline的距离
     */
    private void draw3DText(Canvas canvas, SpannableString text, float rotateX, float offsetY,
        ...
        canvas.save();
        canvas.translate(mStartX, centerY - centerToBaselineY);
        StaticLayout layout = new StaticLayout(text, textPaint, (int) StaticLayout.getDesiredWidth(text, textPaint), Layout.Alignment.ALIGN_NORMAL, 1, 0, false);
        layout.draw(canvas);
        canvas.restore();
     ...
     * @return 文本内容
     */
    protected CharSequence getDataText(T item) {
        if (item == null) {
            return "";
